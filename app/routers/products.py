...
async def list_products(
    ...
    query = select(Product).options(selectinload(Product.consoles)).order_by(case((select(func.count(GameDetail.id_game_detail)).where(GameDetail.producto_id == Product.id_product).where(GameDetail.stock > 0).correlate(Product).scalar_subquery() == 0, 1), else_=0), Product.id_product)
...
async def get_products(offset: int = 0, limit: int = 10, session: AsyncSession = Depends(get_session)):
    ...
    query = select(Product).order_by(case((select(func.count(GameDetail.id_game_detail)).where(GameDetail.producto_id == Product.id_product).where(GameDetail.stock > 0).correlate(Product).scalar_subquery() == 0, 1), else_=0), Product.id_product)
    if offset:
...
async def get_products_by_type(
    ...
    query = (
        ...
        .order_by(case((select(func.count(GameDetail.id_game_detail)).where(GameDetail.producto_id == Product.id_product).where(GameDetail.stock > 0).correlate(Product).scalar_subquery() == 0, 1), else_=0), Product.id_product)
        .limit(limit)
    )
...
async def filter_products(
    ...
    query = query.order_by(case((select(func.count(GameDetail.id_game_detail)).where(GameDetail.producto_id == Product.id_product).where(GameDetail.stock > 0).correlate(Product).scalar_subquery() == 0, 1), else_=0), Product.id_product)
    if offset:
        query = query.offset(offset)
    query = query.limit(limit)
